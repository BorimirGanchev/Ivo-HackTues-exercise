...
function CheckIfEmpty(text: string){
  return text.trim().length == 0;
}

function CheckIfValid(formData: {[key: string]: string}){
  for(const field in formData){
    console.log(formData[field])
    if(CheckIfEmpty(formData[field])){
      return false
    }
  }
...
function Form({ button_text, input_data_arr }: FormProps) {
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    console.log("kur",formData)
    console.log("checking -> ", CheckIfValid(formData))
    const hasEmptyField = Object.values(formData).some((value) => value.trim().length === 0);
    setIsFormEmpty(hasEmptyField);

    if (hasEmptyField || Object.entries(formData).length < input_data_arr.length) {
